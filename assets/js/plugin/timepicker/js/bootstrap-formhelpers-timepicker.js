  ...
  BFHTimePicker.prototype = {
    ...
    initPopover: function() {
      ...
      this.$element.html(
        ...
        iconLeft +
        '<input type="text" maxlength="2" name="' + this.options.name + '" class="' + this.options.input + '" placeholder="' + this.options.placeholder + '" readonly" onfocus="this.blur()" disabled>' +
        iconRight +
        '</div>' +
        ...
        '<tr>' +
        '<td class="hour">' +
        '<input type="text" maxlength="2" class="' + this.options.input + ' bfh-number"  data-min="0" data-max="' + modeMax + '" data-zeros="true" data-wrap="true" onkeypress="if(event.keyCode == 13){ event.preventDefault(); }">' +
        '</td>' +
        '<td class="separator">' + BFHTimePickerDelimiter + '</td>' +
        '<td class="minute">' +
        '<input type="text" class="' + this.options.input + ' bfh-number"  data-min="0" data-max="59" data-zeros="true" data-wrap="true" onkeypress="if(event.keyCode == 13){ event.preventDefault(); }">' +
        '</td>' +
        modeAddon +
        '</tr>' +
        '</tbody>' +
    ...
    change: function(e) {
      ...
      if ($timePicker && $timePicker !== 'undefined') {
        ...
        var sysdate = new Date(),
            ...
            hr = $parent.find('.hour input[type=text]').val(),
            mn = $parent.find('.minute input[type=text]').val();

        if(hr < h2) return false;
        if(hr == h2 && mn < m) return false;
        // if(hr == h && mn < m) mn = (m < 10) ? '0' + m : m;
        // if(hr < h) hr = (h < 10) ? '0' + h : (h2 == 0) ? '12' : (h2 < 10) ? '0' + h2 : h;

        $parent.find('.bfh-timepicker-toggle > input[type="text"]').val(hr + BFHTimePickerDelimiter + mn + mode);
    ...
    toggle: function(e) {
      ...
      if (!isActive) {
        ...
        $this.focus();
        $('.bfh-datepicker').removeClass('open');
      }
