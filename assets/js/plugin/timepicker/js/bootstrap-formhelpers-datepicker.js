  ...
  BFHDatePicker.prototype = {
    ...
    initCalendar: function() {
      ...
      this.$element.html(
        ...
        iconLeft +
        '<input type="text" name="' + this.options.name + '" class="' + this.options.input + '" placeholder="' + this.options.placeholder + '" readonly onfocus="this.blur()" disabled>' +
        iconRight +
        '</div>' +
    ...
    updateCalendarDays: function($calendar, month, year) {
      ...
      for (day=1; day <= numDaysCurrentMonth; day=day+1) {
        if (this.checkMinDate(day, month, year)) {
          ...
          if(day < sysday && month == sysmon && year == sysyear)
            row += '<td data-day="' + day + '" class="off">' + day + '</td>';
          else
    ...
    previousMonth: function () {
      ...
      var sysdate = new Date(),
          mon = sysdate.getMonth(),
          yr = sysdate.getFullYear();

      if(Number($parent.data('year')) < Number(yr)) return false;
      if(Number($parent.data('year')) == Number(yr) && Number($parent.data('month')) < (mon + 1)) return false;
    ...
    previousYear: function () {
      ...
      if(Number($parent.data('year')) < (Number(yr) + 1)) return false;
    ...
    toggle: function (e) {
      ...
      if (!isActive) {
        ...
        $('.bfh-timepicker').removeClass('open'); //customized
      }
