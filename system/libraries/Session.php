...
/**
 * @source 		http://upshots.org/codeigniter/codeigniter-ajax-session-bug-fix
 */
...
class CI_Session {
	...
	public function sess_read()
	{
		...
		if ($this->sess_match_useragent == TRUE && trim($session['user_agent']) !== trim(substr($this->CI->input->user_agent(), 0, 255)))
		{
			$this->sess_destroy();
			return FALSE;
		}
	...
	{
		// We dont want to update session or
		// We only update the session every five minutes by default
		if ($this->sess_time_to_update === 0 || ($this->userdata['last_activity'] + $this->sess_time_to_update) >= $this->now)
		{
			return;
		}
